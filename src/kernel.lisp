...
(defun my-debugger (condition me-or-my-encapsulation)
  ...
  (let* ((restarts (compute-restarts condition))
    ...
    (finish-output *error-output*)
    (do ((res applicable-restarts (cdr res))
         (number-len (length (write-to-string (length applicable-restarts))))
         (name-len (apply #'max (mapcar (lambda (x) (length (symbol-name (restart-name x)))) applicable-restarts)))
         (i 1 (1+ i)))
        ((null res))
      (format *standard-output* "~VD] ~VA  ~A~%" number-len i name-len (restart-name (car res)) (car res)))
    (terpri *standard-output*)
    (finish-output *standard-output*)
    (do ((choice (choose) (choose)))
        ((and (integerp choice)
              (<= 1 choice (length applicable-restarts)))
         (invoke-restart-interactively (nth (1- choice) applicable-restarts))))))
